...
(function () {
  ...
  const bursts = [];
  const matrixCols = [];
  const HEX_CHARS = "01█░▒▓NULL7MIMICHAIKUOPUS";
  ...
  function drawMatrix() {
    ...
    for (const col of matrixCols) {
      ...
      for (let i = 0; i < col.chars.length; i++) {
        ...
        mctx.fillStyle = i === 0 ? `rgba(51,255,136,${alpha + 0.2})` : `rgba(26,122,69,${alpha})`;
  ...
  function drawBursts() {
    for (let i = bursts.length - 1; i >= 0; i--) {
      ...
      ctx.beginPath();
      ctx.shadowBlur = 14;
      ctx.shadowColor = b.hue > 100 ? "#33ff88" : "#c9b458";
